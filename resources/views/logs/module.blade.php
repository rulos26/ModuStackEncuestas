...
@section('title', isset($title) ? $title : 'Errores del Módulo')

@section('content_header')
    <h1>{{ isset($title) ? $title : 'Errores del Módulo' }}</h1>
@endsection

@section('content')
<div class="card">
    <div class="card-header">
        <strong>{{ isset($title) ? $title : 'Errores del Módulo' }}</strong>
    </div>
    <div class="card-body" style="background:#222;color:#eee;font-family:monospace;font-size:13px;max-height:600px;overflow:auto;">
        @if(!empty($log))
            <pre style="white-space: pre-wrap; word-break: break-all;">{{ $log }}</pre>
        @else
