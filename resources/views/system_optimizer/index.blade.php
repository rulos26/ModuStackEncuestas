                    ...
                    <div class="col-md-6">
                        <div class="card bg-success text-white">
                            <div class="card-body text-center">
                                ...
                                <button class="btn btn-light btn-sm" onclick="dumpAutoload()">
                                    <i class="fas fa-play"></i> Ejecutar
                                </button>
                            </div>
                        </div>
...
function showResults(data, title) {
    let html = `<h6><i class="fas fa-check-circle text-success"></i> ${title}</h6>`;
    if (typeof data === 'object') {
        for (const [key, value] of Object.entries(data)) {
            if (value.success) {
                html += `<div class="text-success"><i class="fas fa-check"></i> ${value.message}</div>`;
            } else {
                html += `<div class="text-danger"><i class="fas fa-times"></i> ${value.message}</div>`;
            }
        }
    } else {
        html += `<div>${data}</div>`;
...
function showError(message) {
    $('#results').html(`<i class="fas fa-exclamation-triangle text-danger"></i> ${message}`)
...
function clearCaches() {
    ...
    $.ajax({
        url: '{{ route("system.optimizer.clear-caches") }}',
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            showResults(response, 'Limpieza de Cachés Completada');
        },
        error: function(xhr) {
            showError('Error al limpiar cachés: ' + xhr.responseText);
        },
        complete: function() {
...
function dumpAutoload() {
    ...
    const originalText = showLoading(button);
    $.ajax({
        url: '{{ route("system.optimizer.dump-autoload") }}',
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            if (response.success) {
                showResults(response.message, 'Autoloader Regenerado');
            } else {
                showError(response.message);
            }
        },
        error: function(xhr) {
            showError('Error al regenerar autoloader: ' + xhr.responseText);
        },
        complete: function() {
            restoreButton(button, originalText);
        }
...
function optimizeRoutes() {
    ...
    const originalText = showLoading(button);
    $.ajax({
        url: '{{ route("system.optimizer.optimize-routes") }}',
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            if (response.success) {
                showResults(response.message, 'Rutas Optimizadas');
            } else {
                showError(response.message);
            }
        },
        error: function(xhr) {
            showError('Error al optimizar rutas: ' + xhr.responseText);
        },
        complete: function() {
            restoreButton(button, originalText);
        }
...
function clearTempFiles() {
    ...
    const originalText = showLoading(button);
    $.ajax({
        url: '{{ route("system.optimizer.clear-temp-files") }}',
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            showResults(response, 'Limpieza de Archivos Temporales');
        },
        error: function(xhr) {
            showError('Error al limpiar archivos temporales: ' + xhr.responseText);
        },
        complete: function() {
            restoreButton(button, originalText);
        }
...
function optimizeAll() {
    ...
    const originalText = showLoading(button);
    $.ajax({
        url: '{{ route("system.optimizer.optimize-all") }}',
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            showResults(response, 'Optimización Completa del Sistema');
        },
        error: function(xhr) {
            showError('Error en la optimización completa: ' + xhr.responseText);
        },
        complete: function() {
            restoreButton(button, originalText);
        }
